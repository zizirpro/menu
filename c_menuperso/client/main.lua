...
function RenderPersonalMenu()
	if ESX.PlayerData.job.name == "arena" then
		RageUI.DrawContent({header = true, instructionalButton = true}, function()
			RageUI.Button("Réapprovisionnement de munitions", nil, {}, true,function(h,a,s)
				if s then
					RageUI.CloseAll()
					RemoveAllPedWeapons(GetPlayerPed(-1), true)
					TriggerServerEvent('c_arenawar:weapon')
				end
			end)

			RageUI.Button("~r~Quitter l'arène", nil, {}, true,function(h,a,s)
				if s then
					RageUI.CloseAll()
					TriggerServerEvent('c_arenawar:quite')
				end
			end)
		end)
	else
		RageUI.DrawContent({header = true, instructionalButton = true}, function()
			...
			--[[RageUI.Button('', "" , {RightLabel = "→"}, true , {
				onSelected = function ()
					...
					ESX.ShowNotification("~g~Voici votre vélo, faites en bonne usage.")                          
				end
			})]]--
					...
					if closestDistance ~= -1 and closestDistance <= 3 then
						...
						if not IsPedSittingInAnyVehicle(closestPed) then
							if PersonalMenu.ItemIndex[PersonalMenu.ItemSelected.name] ~= nil and PersonalMenu.ItemSelected.count > 0 then
								if PersonalMenu.ItemSelected.name == "crystal" then
									ESX.ShowNotification("~r~Vous ne pouvez pas donner cet item.")
								else
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Config.Controls.OpenMenu.keyboard) and not Player.isDead and GetLastInputMethod(0) then
